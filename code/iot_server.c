			...
			for (i = 0; i < MAX_CLNT; i++)
			{
				if (!strcmp(client_info[i].id, pArray[0]))
				{
					if (!strcmp(client_info[i].pw, pArray[1]))
					{
						...
						{
							sprintf(msg, "[%s] takeover previous session (old fd:%d)\n",
									pArray[0], client_info[i].fd);
							log_file(msg);
...
void *clnt_connection(void *arg)
{
	...
	while (1)
	{
		...
		strncpy(raw_msg, msg, sizeof(raw_msg) - 1);
		...
		if (raw_msg[0] == '[')
		{
			...
			strncpy(to_msg, raw_msg, sizeof(to_msg) - 1);
			to_msg[sizeof(to_msg) - 1] = '\0';
		}
		else
		{
